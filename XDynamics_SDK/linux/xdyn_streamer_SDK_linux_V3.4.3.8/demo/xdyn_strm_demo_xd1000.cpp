        ...
        if(cfg->isUsed[i] && data[i].addr){
            if(i == MEM_AGENT_SINK_DEPTH){
                ...
                if(data[i].ex){
                    ...
                    printf("params : [%f %f %f %f %f]\n", depthInfo->fLensParas[0], 
                            depthInfo->fLensParas[1], 
                            depthInfo->fLensParas[2], 
                            depthInfo->fLensParas[3], 
                            depthInfo->fLensParas[4]);
                }
...
int main(int argc, char *argv[])
{
    ...
    XDYN_Streamer *stream = CreateStreamerNet(XDYN_PRODUCT_TYPE_XD_1000, EventCB, &userHdl, "192.168.31.3");
    if(stream == nullptr){
