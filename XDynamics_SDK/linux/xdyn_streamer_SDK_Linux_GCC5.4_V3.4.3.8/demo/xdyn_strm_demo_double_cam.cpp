...
void StreamCB(void *handle, MemSinkCfg *cfg, XdynFrame_t *data)
{
    ...
    if(user->count % 100 == 0){
        printf("Device [%d] get stream total %d\n", user->flag, user->count);
    } 
...
int main(int argc, char *argv[])
{
    ...
    UserHandler *usrHdl1 = UserHandle_Create("192.168.71.69", 1);
    UserHandler *usrHdl2 = UserHandle_Create("192.168.71.79", 2);
    ...
    while (!doExit){
        ...
        usleep(1000000);
         ...
         if(count % 200 == 0){
            printf("enable camera 71.69, disable 71.79\n");
            UserHandle_Start(usrHdl1);
            UserHandle_stop(usrHdl2);
        }else if(count % 200 == 100){
            printf("enable camera 71.79, disable 71.69\n");
            UserHandle_stop(usrHdl1);
            UserHandle_Start(usrHdl2);
        }
        count ++;
    }
