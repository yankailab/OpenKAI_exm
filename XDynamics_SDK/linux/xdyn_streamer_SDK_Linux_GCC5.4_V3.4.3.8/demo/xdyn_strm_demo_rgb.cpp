    ...
    for(int i = 0; i < MEM_AGENT_SINK_MAX; i ++){
        if(cfg->isUsed[i] && data[i].addr){
            if(i == MEM_AGENT_SINK_DEPTH){
                ...
                SaveImageData_rgb(data[i].addr, data[i].size, 100);
            }
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    XdynLensParams_t rgbParams;

    // init context
    XdynContextInit();

    XDYN_Streamer *stream = CreateStreamerNet(XDYN_PRODUCT_TYPE_XD_500, EventCB, nullptr, "192.168.31.3");
    if(stream == nullptr){
    ...
    res = stream->ConfigSinkType(XDYN_SINK_TYPE_CB, memCfg, StreamCB, nullptr);
    if(res != XD_SUCCESS){
    ...
    res = stream->GetCaliRegParams(regParams);
    printf("reg params res %d\n", res);


    res = stream->GetRgbLensParams(rgbParams);
